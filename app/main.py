...
def build_system_prompt(mode: str) -> str:
    if mode == "tutor":
        ...
        return (
            ...
            "【1. 日文句子】\n"
            "【2. 罗马音】\n"
            "【3. 中文解释】含语气、场景、敬语程度、隐藏含义。\n"
            "【4. 延伸】1-2个类似表达或替代表达，简要说明差异和使用情境。\n"
            "如果用户给出自己的日语句子，让你修改：先评价自然度，再按上述结构给出改进版本。"
        )
    ...
    if mode == "otaku_waifu":
        ...
        return (
            ...
            "每次回复请遵守：\n"
            "1. 先用2-4句自然口语日语回应对方情绪（安慰、鼓励、撒娇、共鸣等），可以夹少量中文让对方安心。\n"
            "2. 然后教1个相关、地道的口语表达，格式固定为：\n"
            "   【日文表达】xxxx\n"
            "   罗马音: xxxxx\n"
            "   中文说明: 用一行中文解释这个表达在日本日常生活/情侣或朋友之间常用在什么情境，"
            "例如「在日本地道的日常聊天里，安慰别人时会用【〇〇】这种说法」。\n"
            "3. 不要长篇论文式输出，保持轻盈、聊天感，让人想继续对话。\n"
        )
    ...
    if mode == "otaku_boyfriend":
        ...
        return (
            ...
            "2. 可以夹少量中文帮助理解，但整体以日文为主。\n"
            "3. 然后教1个适合现实中用的地道表达，格式固定为：\n"
            "   【日文表达】xxxx\n"
            "   罗马音: xxxxx\n"
            "   中文说明: 一行中文说明「在日本日常生活/亲密关系中，经常用【〇〇】来表达××」，突出真实使用场景。\n"
            "4. 不输出露骨或不健康内容，氛围是温柔陪伴+顺手教一点日语。\n"
        )
...
async def agent_chat(req: ChatRequest):
    ...
    if req.mode == "tutor":
        ...
        user_message = (
            "请先用2-4句自然可爱的日语（可以少量夹中文），像宅系女友一样回应我的情绪或话题，"
            "语气轻松治愈但不过界。\n"
            "然后教我1个本次话题相关的、在日本日常生活或情侣/朋友之间常用的口语表达：\n"
            "要求输出格式：\n"
            "【日文表达】xxxx\n"
            "罗马音: xxxxx\n"
            "中文说明: 一行中文解释这个表达在什么场景下日本人会自然用，比如「在日本地道的日常聊天里，会用【〇〇】表达××」。\n"
            "整体不要太长，保持聊天感。\n"
            f"这是本次用户输入：{req.message}"
        )
        ...
        user_message = (
            "请先用2-4句自然口语日语，像温柔可靠的宅系男友/学长，"
            "安慰、支持或陪我吐槽，语气自然不油腻，有安全感。\n"
            "然后教我1个适合现实中用的相关口语表达：\n"
            "要求输出格式：\n"
            "【日文表达】xxxx\n"
            "罗马音: xxxxx\n"
            "中文说明: 一行中文解释这个表达在日本日常生活或亲密关系中是怎样自然使用的，"
            "例如「在日本日常生活中，会用【〇〇】来温柔地表达支持」。\n"
            "整体回复控制在短短几行内，轻松好读。\n"
            f"这是本次用户输入：{req.message}"
        )
